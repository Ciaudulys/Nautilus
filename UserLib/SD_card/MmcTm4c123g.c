...
#define SDC_SSI_FSS_PORT_BASE     	GPIO_PORTF_BASE
#define SDC_SSI_FSS_SYSCTL_PERIPH  	SYSCTL_PERIPH_GPIOF
...
void SELECT (void)
{
    GPIOPinWrite(SDC_SSI_FSS_PORT_BASE, SDC_SSI_FSS, 0);
}
...
void DESELECT (void)
{
    GPIOPinWrite(SDC_SSI_FSS_PORT_BASE, SDC_SSI_FSS, SDC_SSI_FSS);
}
...
void send_initial_clock_train(void)
{
    ...
    GPIOPinWrite(SDC_GPIO_PORT_BASE, SDC_SSI_TX, SDC_SSI_TX);
		//GPIOPinWrite(SDC_GPIO_PORT_BASE, SDC_SSI_TX, 0);
		//GPIOPinWrite(SDC_GPIO_PORT_BASE, SDC_SSI_TX, 0xFF);
    ...
    GPIOPinTypeSSI(SDC_GPIO_PORT_BASE, SDC_SSI_TX);
		GPIOPinConfigure(GPIO_PD3_SSI3TX);				/* Initialize port pin tied to DO */
}
...
void power_on (void)
{
    ...
    SysCtlPeripheralEnable(SDC_GPIO_SYSCTL_PERIPH);
		SysCtlPeripheralEnable(SDC_SSI_FSS_SYSCTL_PERIPH);
     ...
     */
		...
		GPIOPinTypeGPIOOutput(SDC_SSI_FSS_PORT_BASE, SDC_SSI_FSS);
		GPIOPadConfigSet(SDC_SSI_FSS_PORT_BASE, SDC_SSI_FSS,
                         GPIO_STRENGTH_10MA, GPIO_PIN_TYPE_STD);
		DESELECT();
	
    GPIOPinTypeSSI(SDC_GPIO_PORT_BASE, SDC_SSI_TX | SDC_SSI_RX | SDC_SSI_CLK);
	
		GPIOPinConfigure(GPIO_PD0_SSI3CLK);		/* Initialize port pin tied to SCLK */
		GPIOPinConfigure(GPIO_PD2_SSI3RX);				/* Initialize port pin tied to DI */
		GPIOPinConfigure(GPIO_PD3_SSI3TX);				/* Initialize port pin tied to DO */
    ...
    GPIOPadConfigSet(SDC_GPIO_PORT_BASE, SDC_SSI_CLK | SDC_SSI_TX,
